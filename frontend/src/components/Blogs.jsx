...
function Blogs() {
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:10000/api/blogs").then((res) => {
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`http://localhost:10000/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("http://localhost:10000/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
